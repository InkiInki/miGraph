...
def test_10cv():
    """
    """
    file_name = "D:/Data/OneDrive/文档/Code/MIL1/Data/Drug/musk1+.mat"
    ...
    for i in range(loops):
        mil = miGraph(file_name)
        classifier = Classify(["knn", "svm"], ["f1_score", "acc", "roc"])
    ...
    print("%.4lf %.4lf %.4lf %.4lf %.4lf %.4lf "
          "%.4lf %.4lf %.4lf %.4lf %.4lf %.4lf " % (np.sum(te_f1_k) / loops, np.std(te_f1_k),
                                                    ...
                                                    np.sum(te_roc_s) / loops, np.std(te_roc_s)), end="")
                                                    # np.sum(te_f1_j) / loops, np.std(te_f1_j),
                                                    # np.sum(te_acc_j) / loops, np.std(te_acc_j),
                                                    # np.sum(te_roc_j) / loops, np.std(te_roc_j))
